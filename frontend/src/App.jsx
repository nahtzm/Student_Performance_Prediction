...
const App = () => {
  ...
  const columns = [
    ...
    { title: "Điểm G1", dataIndex: "previous_score", key: "previous_score" },
      ...
      <div className="content-wrapper">
        <Card className="prediction-card">
          ...
          <Form
            layout="vertical"
            onFinish={onFinish}
            initialValues={{ failures: 0, goout: 3 }}
          >
            <Row gutter={16}>
              <Col span={8}>
                <Form.Item
                  label="Số giờ học/tuần"
                  name="study_hours"
                  rules={[{ required: true }]}
                >
                  <InputNumber
                    ...
                    placeholder="Thang 1-4"
                  />
                </Form.Item>
              </Col>
              <Col span={8}>
                <Form.Item
                  label="Điểm kỳ trước (G1)"
                  name="previous_score"
                  rules={[{ required: true }]}
                >
                  <InputNumber
          ...
          {predictionData && (
            <div
              className={`result-box ${predictionData.prediction === "PASS" ? "pass" : "fail"}`}
            >
              <Title level={4} className="result-status">
                Kết quả: {predictionData.prediction}
              </Title>
              <Text strong className="result-score">
                Điểm số dự kiến: {predictionData.score}/20
              </Text>{" "}
              <br />
              <Text italic className="result-advice">
                Lời khuyên: {predictionData.advice}
              </Text>
            </div>
          )}
        </Card>
